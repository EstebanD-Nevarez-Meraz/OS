...
void Scheduler::srtfSchedule(ReadyQueue* readyQueue) {
    ...
    while (n_complete != n_process) {
        ...
        for (int i = 0; i < n_process;) {
            ...
            for (int z = 0; z < n_process; z++) {
                if (processList[i].remaining_time == 0) {
                    finishTimes[i] = current_time + (contextT * .5);
                    sts = i + 1;
                }
                else if (processList[i].remaining_time > processList[z].remaining_time && processList[z].arrivalTime <= current_time)
                {
                    //When the remaining time for another task is shorter than current tasks remaining time
                    //0.5 for context switching
                    contextT += 1;
                    processList[i].num_contexts++;
                    // change to new task
                    ...
                    current_time += 1;
                    processList[i].remaining_time -= 1;
                    sts = i;
                }
            }
            i = sts;
